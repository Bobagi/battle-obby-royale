...
local function resolveSetActionRemote(): RemoteEvent?
        local gameplayCoreFolder = ReplicatedStorage:FindFirstChild("GameplayCore")
        if not gameplayCoreFolder then
                return nil
        end

        local movementFolder = gameplayCoreFolder:FindFirstChild("Movement")
        if not movementFolder then
                movementFolder = gameplayCoreFolder:WaitForChild("Movement")
        end

        local remotesFolder = movementFolder:FindFirstChild("Remotes")
        if not remotesFolder then
                remotesFolder = movementFolder:WaitForChild("Remotes")
        end

        return remotesFolder:FindFirstChild("SetAction") or remotesFolder:WaitForChild("SetAction")
end
...
function Controller.new(character: Model)
        ...
        character:SetAttribute(Constants.ACTION_ATTRIBUTE, character:GetAttribute(Constants.ACTION_ATTRIBUTE) or "None")
        character:SetAttribute(Constants.CAN_DASH_ATTRIBUTE, true)
        character:SetAttribute(Constants.CAN_DOUBLE_JUMP_ATTRIBUTE, true)
        character:SetAttribute(Constants.LAST_GROUNDED_ATTRIBUTE, os.clock())
        return self
end
...
function Controller:setAction(action: string)
        ...
        if setActionRemote then
                ...
                warn("SetAction remote missing; actions will not replicate")
        end
end
